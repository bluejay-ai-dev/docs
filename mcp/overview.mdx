...
<CardGroup cols={2}>
  <Card title="Claude Code & Claude Desktop" icon="terminal" href="/mcp/claude-code">
    Set up Bluejay MCP in Claude Desktop or Claude Code CLI
  </Card>
  <Card title="Cursor & Windsurf" icon="code" href="/mcp/cursor-windsurf">
...
| `queue_email_simulation_run` | Queue an email simulation run |
...
| Tool | Description |
|------|-------------|
| `create_prompt_version` | Create a new agent version |
| `get_prompt` | Fetch an agent version |
| `list_prompts` | List all agent versions |
| `list_prompt_versions` | List an agent's versions |
| `get_prompt_version` | Fetch a specific agent version |
| `add_prompt_version_labels` | Add labels to an agent version |
| `get_prompt_version_labels` | Get labels for an agent version |
| `delete_prompt_version_labels` | Remove labels from an agent version |
| `delete_prompt` | Delete an agent version by unique ID |
| `delete_prompt_version` | Delete an agent version by version number |
...
| Tool | Description |
|------|-------------|
| `create_alert` | Create an alert on a metric |
...
| `update_uptime_monitor` | Partial update — checks, notifications, frequency, pause/resume |
...
| `get_uptime_monitor_series` | Uptime % and p50/p90/p95/p99 response latency per hour, day, week, or month |
...
| Tool | Description |
|------|-------------|
| `sync_agent_workflow` | Pull the latest workflow from the provider |
| `push_agent_workflow` | Push edited workflow back to the provider |
| `get_workflow_summary` | Fetch the synced workflow graph |
| `list_workflow_paths` | Enumerate distinct paths through the workflow |
| `add_workflow_node` / `update_workflow_node` / `delete_workflow_node` | Edit nodes |
| `add_workflow_edge` / `delete_workflow_edge` | Edit edges |

### Self-Improve

| Tool | Description |
|------|-------------|
| `start_self_improve_run` | Start an autonomous prompt-improvement loop |
| `get_self_improve_run` | Fetch run status and per-round history |
| `list_self_improve_runs` | List runs |
| `stop_self_improve_run` | Stop a run |

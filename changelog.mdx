...
<Update label="September 9th, 2026" tags={["Improvement", "API"]}>
  ...
  Agent versions now live at `/v1/agents/{id}/versions`. The `/v1/agents/{id}/prompts` paths stay as HTTP aliases. Evaluate accepts an optional integer `version` next to `label`. An unknown version returns 404. An unknown label is created. When both are sent, version wins for the stored agent version.

...
Vapi can stream its Langfuse traces to Bluejay. Point Vapi's Langfuse export at Bluejay once and every call gets a trace on the **Traces** tab: one generation per model turn with the model, parameters, prompt, output, time to first token and end time, and the tool calls the model requested.

1. In Vapi, go to **Provider Credentials**, then **Observability Providers**, then **Langfuse**.
2. Fill in:
...
| **Secret Key** | your Bluejay API key |

3. Save. Vapi verifies the host immediately and starts sending traces for every call on the account.

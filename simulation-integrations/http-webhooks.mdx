---
title: "Chat Testing"
description: "Test text-based chat agents using HTTP webhooks for bidirectional messaging"
icon: webhook
---

<iframe
  src="https://www.loom.com/embed/49ce062677ed44be932c5dd0a89c7269?speed=1.5&captions=true&hideEmbedTopBar=true"
  width="100%"
  height="450"
  frameBorder="0"
  allowFullScreen
...
sequenceDiagram
    ...
    participant Bluejay's Digital Human
   
    Your Agent->>Bluejay's Digital Human: Send Message (HTTP)
    Bluejay's Digital Human->>Your Agent: Response (Webhook)
```
...
sequenceDiagram
    ...
    Your Agent->>Bluejay's Digital Human: API Call: "queue-http-text-simulation-run"
    Bluejay's Digital Human-->>Your Agent: Response: "simulation_result_ids"
    loop Conversation
        Your Agent->>Bluejay's Digital Human: API Call: "send-http-text-message"
        Bluejay's Digital Human->>Your Agent: Response sent to configured Webhook
    end
    Bluejay's Digital Human->>Bluejay's Digital Human: Evaluate conversation
```
...
curl -X POST https://api.getbluejay.ai/v1/add-agent \
  ...
  -d '{
    ...
    "http_webhook": "https://your-server.com/webhook"
  }'
```
</Tab>
</Tabs>
...
To ensure webhook requests are genuinely from Bluejay, verify the signature included in each request. Bluejay signs the raw request body with HMAC-SHA256 using your signing key (generated in the agent's Connection settings) and sends the hex digest in the `X-Bluejay-Signature` header.

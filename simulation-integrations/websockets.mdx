...
Bluejay sends `X-Simulation-Result-Id` as an HTTP header on the WebSocket upgrade request. Capture it if you want to POST tool calls or traces back to [`/v1/update-simulation-result`](/api-reference/endpoint/update-simulation-result) after the call.

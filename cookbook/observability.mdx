...
The most efficient way to receive evaluation results is through the [events webhook](/api-reference/webhook/events-webhook). When you submit a call for evaluation, Bluejay will automatically send the complete results to your registered webhook URL once processing is complete.
...
For webhook registration and more advanced use cases, explore the [API Reference](/api-reference/add-agent) or ask in our [Slack community](https://join.slack.com/t/bluejaycommunity/shared_invite/zt-442xk85k5-Qkbq65wVOxK~1FHU5OadEA) for custom evaluation configurations.

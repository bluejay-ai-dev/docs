...
2. It polls until the run finishes (`GET /v1/retrieve-simulation-results/{run_id}`). The score uses `tests_passed` and `total_tests` on `simulation_run`, which is the full run even though `simulation_results` is now one page of at most 100 conversations.
...
| `BLUEJAY_PROMPT_ID` | Agent version override ID (the action input is still named `prompt_id`) | No |
...
| `prompt_id` | No | — | Deprecated. Same uuid as `agent_version_id`. Override the agent version for this run. |

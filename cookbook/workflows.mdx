...
Workflows in Bluejay are visual representations of conversation flows that define how your agent should handle different customer interaction paths. They consist of agent nodes (define what your agent should say or do so simulations can evaluate against it) and user nodes (script the digital human's side of the conversation for simulations) that create a structured map of possible conversation outcomes.

When you run a simulation from a workflow, Bluejay identifies every unique path through the graph and turns each one into its own digital human. A workflow with two branches produces two digital humans — each simulating a different route through the conversation. This means the structure of your workflow directly determines the coverage of your simulation.
...
A Workflows definition is a **React Flow graph** with two arrays:

- **`nodes`** — individual conversation steps
- **`edges`** — directed connections between nodes

Both arrays are optional on create (an empty graph is saved as a draft). Full validation only runs once your graph contains at least one content node.
...
Options nodes model a point where the user picks from multiple paths — pressing a key, saying a specific phrase, or staying silent. Each branch is defined in `data.branch_options` (minimum 2 entries). Options nodes are always user-only.

Each branch option has:
- `id` — a unique string used to match outgoing edges via `sourceHandle`
- `type` — `exact`, `silence`, or `dtmf` (not `contextual`)
        ...
        "nodes": [
            ...
            },
            {
                ...
                "data": {
                    "type": "contextual",
                    "speaker": "agent",
                    "message": "Listen to the customer's issue and acknowledge their concern before resolving it."
                }
            },
            {
                "id": "agent-resolution",
...
def create_branching_workflow(api_key, agent_id=None):
    ...
    Branch 1: user presses 1 → customer support
    Branch 2: user presses 2 → billing support
    """
        ...
        "nodes": [
            ...
            },
            {
                ...
                "data": {
                    "type": "exact",
                    "speaker": "agent",
                    "message": "Press 1 for customer support, or press 2 for billing."
                }
            },
...
Once your workflow is created, you can use it to automatically generate digital humans — one per unique path through the graph. Pass the workflow's UUID and a count to `workflow_adherence_v2` on the `POST /v1/generate-digital-humans` endpoint.

Bluejay enumerates every distinct start-to-leaf path through the graph and generates a digital human for each one. The `count` in the dict is how many digital humans you want; if the workflow has fewer paths than the count only the available paths are used. Counts above **200 are silently capped at 200** — the call never fails due to a high count, it just generates at most 200 digital humans per workflow.
...
- **Draft first** — send an empty `definition: {}` to create the workflow row, then update it incrementally with `PUT` as you build the graph

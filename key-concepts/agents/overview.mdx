...
| **Connection** | How Bluejay reaches the agent: a connection type (Phone, SIP, LiveKit, Websocket, ElevenLabs, Retell, VAPI, Google CES, Dialogflow CX, and more) plus its details, like a phone number or provider agent ID. Required for simulations to connect and for observability webhooks to be matched correctly. |

## Creating an Agent

You can create an **Agent** from the Bluejay dashboard or from the API.

- **Dashboard.** Click **Create Agent**, pick the agent type (Inbound or Outbound), mode (Voice or Text), and connection type, then fill in the connection details.
...
- **Workflows** (for ElevenLabs, Vapi, Retell, and Google Dialogflow CX agents) sync into Bluejay and render as an editable graph on the Agent's Workflow tab.
...
2. **Configure** the Agent by setting goals and attaching a knowledge base.
3. **Connect** the Agent to its production system by choosing the connection type and entering the phone number or provider agent ID.
...
Connection: Retell (synced agent)
```
...
<CardGroup cols={2}>
  <Card title="Workflows" icon="diagram-project" href="/key-concepts/agents/workflow">
    Sync multi-agent flows from your provider and use them to drive simulations.
  </Card>

...
description: Run every digital human in a simulation under several delivery conditions at once
icon: masks-theater
---

A personality is a way of calling: a language, a noisy line, a caller who cuts the agent off. You pick them on a simulation run, and every digital human in that run calls as itself **and** once per personality — same scenario, same success criteria, different delivery.
...
flowchart LR
    ...
    P["Personalities:<br/>Spanish · Interruptions"]

    DH1 --> A1[as itself]
    DH1 --> A2[Spanish]
    DH1 --> A3[Interruptions]
    DH2 --> B1[as itself]
    DH2 --> B2[Spanish]
    DH2 --> B3[Interruptions]
    P -.-> Run
```
...
<AccordionGroup>
  <Accordion title="Voice" icon="microphone">
    Language, accent and gender. Set as a group, so a personality that switches to Spanish still sounds like a real caller.
  </Accordion>
...
Personalities are shared across your organization, so one called "Spanish" is defined once and any simulation can select it. The selection is saved on the simulation, so the next run — including a scheduled one — starts with the same set.

| Preset | What it overrides |
| --- | --- |
| Spanish | The voice, matching the caller's gender |
...
<Tip>
Presets are starting points, not fixed types. Edit any field afterwards and rename it: "Spanish caller on a bad line who interrupts" is a single personality.
</Tip>
...
Because every run includes the unmodified call, you always have a baseline: if the base passes and Spanish fails, the scenario is fine and the language handling is not.

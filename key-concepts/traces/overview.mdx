...
### Using a Langfuse exporter

Bluejay accepts the Langfuse ingestion API, so anything that can send traces to a self-hosted Langfuse can send them to Bluejay instead. Use these values wherever the exporter asks for a Langfuse host and keys:

```
Host:       https://api.getbluejay.ai/v1/langfuse
Public key: bluejay
Secret key: <your Bluejay API key>
```

Vapi's built-in Langfuse export works this way, see the [Vapi guide](/simulation-integrations/vapi#traces-llm-latency-prompts-and-tool-calls).

---
title: Metric Types
description: Understand the response types available for Custom Metrics in Bluejay
icon: shapes
---
...
To create this metric via API, use the [Create Custom Metric](/api-reference/endpoint/create-custom-metric) endpoint with `response_type` set to `tool_call` and the expected names in `tool_names`.
...
| `tool_call` | `pass` or `fail` (deterministic) | Verifying required tools were called | Active |

...
| **Agent Audio Dropouts** | The agent's audio cut out mid-speech and then continued. Voice only, off until you turn it on |
...
Agent Audio Dropouts counts an abrupt hole inside one agent turn: the audio collapses, stays gone, then the agent keeps talking. A pause the agent produces, silence between turns, and the agent hanging up are not dropouts. The check fails when the probe records any dropout. It is skipped on SMS, email, and webchat.

...
flowchart LR
    ...
    Fire --> Notify[Slack / Email / Webhook]
...
| **Operator** | Whether to alert when the score is _above_ or _below_ the boundary | Above |
| **Value** | The numeric boundary that counts as a violation | 3 seconds |
| **Notify When** | The minimum number of matching events before the alert fires | 5 |
| **Threshold Interval** | The rolling interval over which matching events are counted | 10 minutes |
...
To kick off an automated flow when the alert fires (triage, ticket, workflow), subscribe a URL under [Settings → Webhooks](/key-concepts/webhooks/overview) to **Observability alert triggered**. That is separate from Slack/email on the alert itself.

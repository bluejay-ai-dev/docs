...
<Steps titleSize="h3">
  ...
  <Step title="Set up alerts">
    ...
    To create an alert:
    1. Go to **Monitor → Alerts** and click **Create Alert**
...
<Tip>
**Hands-on exercise:** Create a Custom Metric called "Agent introduction" with a pass/fail response type and the prompt: *"Did the agent introduce itself by name at the start of the conversation?"* Then create an alert on this metric that fires when the pass rate drops below 80%. If you have Slack connected, route the alert to a test channel. Verify the alert appears on the **Monitor → Alerts** page.
</Tip>

...
if run gcloud iam service-accounts add-iam-policy-binding "$SA" --project="$PROJECT_ID" \
  ...
  # Hand the grant back even if you Ctrl-C during the wait below. Only counted as
  # dropped once Google says so, otherwise the EXIT trap gets another go and you
  # get the command to run yourself rather than a grant quietly left behind.
  DROPPED=0
  drop_grant() {
    if [[ $DROPPED == 1 ]]; then return 0; fi
    if run gcloud iam service-accounts remove-iam-policy-binding "$SA" --project="$PROJECT_ID" \
         --role=roles/iam.serviceAccountTokenCreator --member="$ME_MEMBER" --condition=None --quiet; then
      DROPPED=1
    else
      echo "could not remove the temporary impersonation grant. Remove it yourself with:" >&2
      echo "  gcloud iam service-accounts remove-iam-policy-binding $SA --project=$PROJECT_ID --role=roles/iam.serviceAccountTokenCreator --member=$ME_MEMBER --condition=None" >&2
    fi
  }
  ...
  echo
  drop_grant
  if [[ $DROPPED == 1 ]]; then trap - EXIT INT TERM; fi
  if [[ $VERIFIED == 1 ]]; then

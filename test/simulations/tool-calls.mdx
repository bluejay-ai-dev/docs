...
<Note>
  The [`/v1/update-simulation-result`](/api-reference/endpoint/update-simulation-result) endpoint also accepts `trace_ids` for linking OpenTelemetry traces to simulation results. See [Traces](/key-concepts/traces/overview#linking-traces-to-simulation-results) for details.
</Note>
...
Bluejay supports four methods for tool call tracking, depending on your infrastructure.
...
WebSocket integration using the CHIRP protocol provides real-time, bidirectional communication. The `X-Simulation-Result-Id` is sent as an HTTP header on the WebSocket upgrade request.
...
**Best for**: agents hosted on Vapi and tested by dialing their phone number.

Connect Vapi with your private key (see the [Vapi Integration Guide](/simulation-integrations/vapi)) and Bluejay fetches each simulated call from Vapi after it ends. Tool calls appear on the **Tool Adherence** tab and a trace of the call (turns and tool executions) on the **Traces** tab, with no changes to your agent.

**Flow:**
1. Add your Vapi private key under **Integrations**
2. Run simulations against the agent's Vapi phone number as usual
3. After each call ends, Bluejay matches it to the Vapi call record and attaches the tool calls and trace

Bluejay matches on the number it dialed from, the call's start and end times, and the transcript, so concurrent calls to the same number are attributed correctly. The data lands a few minutes after the call ends, once Vapi has finished writing the call record.

### Why SIP for Phone-Based Agents?

If your agent runs through traditional phone systems (PSTN), basic telephony integration does **not** support tool call enrichment. To unlock tool call and metadata tracking for phone-based agents, you need SIP. Agents hosted on Vapi are the exception: Bluejay reads their tool calls from Vapi directly, as described above.
...
- **Vapi-hosted agents** → connect Vapi under **Integrations**; nothing else to do
...
def track_tool_call(name, parameters, output, start_offset_ms):
    return {
        "name": name,                      # required: the tool that was invoked
        "parameters": parameters,          # optional: input arguments
        "output": output,                  # optional: return value
        "start_offset_ms": start_offset_ms # required: milliseconds from call start
    }
```
  ...
  "events": [
    {
      ...
      "description": "Voice biometric check passed",
...
<CardGroup cols={2}>
  ...
  <Card title="Traces" icon="route" href="/key-concepts/traces/overview#linking-traces-to-simulation-results">

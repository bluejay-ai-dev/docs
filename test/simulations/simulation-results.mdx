...
Once a [simulation run](/test/simulations/simulation-runs) completes, Bluejay generates a detailed results report. This page explains the metrics, call-level data, and best practices for interpreting those results so you can turn raw data into actionable improvements.
...
- **Call-Specific Metadata**
  - Contextual data points such as timestamps, latency measurements, and tags. These help you filter and compare calls within a run.

## Personalities

If the run has [personalities](/key-concepts/digital-humans/personalities), every digital human calls as itself and once per personality, and those conversations group into one row with a badge counting them. Expand the row to compare the base run against each personality, and open a conversation to see the personality it ran as in the side panel's **Digital Human** tab.

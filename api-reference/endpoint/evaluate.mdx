...
<Tip>
  `label` and `version` are optional. An unknown `label` creates a labels row. An unknown integer `version` returns 404 and is never created. When both are sent, `version` wins for the stored agent version. The `label` is still created if it does not exist. Omit `label` on the `agent_id` path to leave it unset. The `external_agent_id` path defaults `label` to `production`.
</Tip>

...
<Note>
  ...
  **New:** optional integer `version`. `label` still works. An unknown version returns 404. An unknown label is created. When both are sent, `version` is the one stored on the call. Omit `label` on the `agent_id` path to leave it unset. The `external_agent_id` path defaults `label` to `production`.
</Note>

...
The conversation includes `transcript` (turns from the database) when stored inline. `transcript_url` is only set when the transcript is not in the database. Prefer `transcript`, then fall back to downloading `transcript_url`. 

...
| digital_human | object | Digital human data model for requests (without id and created_at). |
...
- Include error handling for **400** (workflow-tagged / enriched-playback rules) and **422** (validation).
...
This endpoint allows you to create a digital human and attach it to a simulation. Workflow-tagged test cases and non-empty **enriched_playback** change which fields are allowed and how intent/transcripts are processed — see [Workflow tags & enriched playback](/key-concepts/digital-humans/workflow-and-enriched-playback).

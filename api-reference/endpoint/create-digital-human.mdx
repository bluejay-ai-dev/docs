...
> **Note:** `endpointing_delay` is referenced as **patience** in the UI. It controls how long (in seconds) the Digital Human waits after silence before assuming the speaker has finished talking. On an SMS Digital Human it is the minimum wait (in seconds) before a reply is sent, counting the time spent composing it.

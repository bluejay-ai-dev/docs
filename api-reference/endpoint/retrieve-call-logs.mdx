...
description: "Retrieve an agent's call logs, newest first, each with its transcript."
...
> **What this endpoint does:** Returns an agent's call logs, newest first, paged. Each call log includes its diarized `transcript`, evaluations, tool calls and `metrics`.
...
Returns call logs for `agent_id`, newest first. Page through with `page` and `page_size` (up to 100), and use `last=all` for the full history.

Each call log includes `transcript`: the diarized transcript, one entry per utterance with `speaker`, `utterance`, `start_offset_ms` and `end_offset_ms`.
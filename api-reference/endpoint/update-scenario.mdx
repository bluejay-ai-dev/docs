...
description: 'Partially update a scenario; replacing definition re-validates the same typed React Flow graph as create.'
openapi: 'PUT /v1/scenario/{workflow_id}'
---
...
Omitted fields are left unchanged. If `agent_ids` appears in the body (including `[]`), all agent links are **replaced**; `[]` clears links. If `definition` appears, it is **fully replaced** and validated like **POST** `/v1/scenario` (same discriminated node and `data` types as [Create scenario](/api-reference/endpoint/create-scenario)).

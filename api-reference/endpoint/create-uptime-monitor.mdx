...
The template checks (`checkType` of `connection`, `agent_malfunction`, `latency`, or `agent_audio_dropouts`) have `isTemplate: true`. `agent_audio_dropouts` is voice only and fails when the probe records any abrupt hole in the agent's audio. A user-added check has `isTemplate: false` and points at a custom metric (`customMetricId`, with a `condition` of `data_type`, `operator`, and `comparison_value`) or a Digital Human (`digitalHumanId`).
